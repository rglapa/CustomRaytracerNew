                               ...
                               texture2d<float>            rtReflections     [[texture(AAPLTextureIndexReflections), function_constant(is_raytracing_enabled)]])
{
    ...
    LightingParameters params = calculateParameters(in, cameraData, lightData, pSubmesh->materials[AAPLTextureIndexBaseColor], pSubmesh->materials[AAPLTextureIndexNormal], pSubmesh->materials[AAPLTextureIndexMetallic], pSubmesh->materials[AAPLTextureIndexRoughness], pSubmesh->materials[AAPLTextureIndexAmbientOcclusion], skydomeMap);
    ...
    if (tid.x < w && tid.y < h)
    {
        ...
        else
        {
            ...
            if (intersection.type == raytracing::intersection_type::triangle)
            {
                ...
                {
                    constant uint16_t* pIndices = (constant uint16_t *)submesh.indicies;
                ...
                {
                    constant uint32_t* pIndices = (constant uint32_t *)submesh.indicies;
                    i0 = pIndices[intersection.primitive_id * 3 + 0];
                    i1 = pIndices[intersection.primitive_id * 3 + 1];
                    i2 = pIndices[intersection.primitive_id * 3 + 2];
                }
                ...
                texture2d<float> baseColorMap         = submesh.materials[AAPLTextureIndexBaseColor];
                texture2d<float> normalMap            = submesh.materials[AAPLTextureIndexNormal];
                texture2d<float> metallicMap          = submesh.materials[AAPLTextureIndexMetallic];
                texture2d<float> roughnessMap         = submesh.materials[AAPLTextureIndexRoughness];
                texture2d<float> ambientOcclusionMap  = submesh.materials[AAPLTextureIndexAmbientOcclusion];
                ...
                LightingParameters params = calculateParameters(colorIn, cd, lightData, baseColorMap, normalMap, metallicMap, roughnessMap, ambientOcclusionMap, skydomeMap);
