...
typedef struct ThinGBufer
{
    id<MTLTexture> positionTexture;
    id<MTLTexture> directionTexture;
} ThinGBuffer;

/// A helper funciton that passes an array of instances into batch methods that require pointers and counts.
...
- (nonnull instancetype) initWithMetalKitView:(MTKView *)view size:(CGSize)size
{
    self = [super init];
    if (self)
    {
...
{
    MTLTextureDescriptor* desc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRG11B10Float width:size.width height:size.height mipmapped:YES];
...
- (void)loadAssets
{
    ...
    // Create a Model I/O vertexDescriptor to format the Model I/O mesh verticies to
    // fit the Metal render pipeline's vertex descriptor layout.
    MDLVertexDescriptor *modelIOVertexDescriptor = MTKModelIOVertexDescriptorFromMetal(_mtlVertexDescriptor);
    ...
    MDLVertexDescriptor *skyboxModelIOVertexDescriptor = MTKModelIOVertexDescriptorFromMetal(_mtlSkyboxVertexDescriptor);
...
- (void)buildSceneArgumentBufferFromReflectionFunction:(nonnull id<MTLFunction>)function
{
    ...
    // Create argument buffer encoders from the scene argument of the ray-tracing function.
    id<MTLArgumentEncoder> sceneEncoder = [function newArgumentEncoderWithBufferIndex:SceneIndex];
    
    id<MTLArgumentEncoder> instanceEncoder = [sceneEncoder newArgumentEncoderForBufferAtIndex:AAPLArgumentBufferIDSceneInstances];
    
    id<MTLArgumentEncoder> meshEncoder = [sceneEncoder newArgumentEncoderForBufferAtIndex:AAPLArgumentBufferIDSceneMeshes];
    
    id<MTLArgumentEncoder> submeshEncoder = [meshEncoder newArgumentEncoderForBufferAtIndex:AAPLArgumentBufferIDMeshSubmeshes];
    ...
    // Encode the instances in the `Scene` (`Scene::instances`).
    ...
    for (NSUInteger i = 0;i < _meshes.count; ++i)
    {
        ...
        {
            AAPLSubmesh* submesh = mesh.submeshes[i];
        ...
        [meshEncoder setBuffer:submeshArgumentBuffer
                        offset:0
                       atIndex:AAPLArgumentBufferIDMeshSubmeshes];
    }
    ...
    // Set `Scene::instances`.
    [sceneEncoder setBuffer:instanceArgumentBuffer offset: 0 atIndex:AAPLArgumentBufferIDSceneInstances];
...
- (void)buildSceneArgumentBufferMetal3 NS_AVAILABLE(13, 16)
{
    ...
    for (NSUInteger i = 0;i<_meshes.count; ++i)
    {
        ...
        NSAssert(metalKitMesh.vertexBuffers.count == 2, @"unkown number of buffers!");
...
- (id<MTLAccelerationStructure>)allocateAndBuildAccelerationStructureWithDescriptor:(MTLAccelerationStructureDescriptor *)descriptor commandBuffer:(id<MTLCommandBuffer>)cmd
{
    ...
    id<MTLBuffer> scratch = [_device newBufferWithLength:sizes.buildScratchBufferSize
...
/// Calculate the minimum size needed to allocate a heap that contains all acceleration structures fro the passed-in descriptors.
    ...
    if ((opts == NULL) || (strstr(opts, "1") != opts))
    {
        if (@available(macOS 13, iOS 16, *))
        {
            if ([_device supportsFamily:MTLGPUFamilyMetal3])
            {
                ...
                primitiveAccelerationStructures = [self allocateAndBuildAccelerationStructuresWithDescriptors:primitiveAccelerationDescriptors heap:accelerationStructureHeap maxScratchBufferSize:storageSizes.buildScratchBufferSize signalEvent:_accelerationStructureBuildEvent];
            }
        }
    }
...
- (void)buildSceneResidencySet NS_AVAILABLE(15, 18)
{
    ...
    id<MTLResidencySet> sceneResidencySet = [self newResidencySetWithLabel:@"sceneResidencySet" error:&error];
    ...
    {
        [sceneResidencySet addAllocations:data count:count];
    };
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    {
        // When ray tracing is an enabled state, first render a thin G-Buffer
        ...
        if ( _renderMode == RMMetalRaytracing || _renderMode == RMReflectionsOnly)
        {
            ...
            renderEncoder.label = @"TinGBufferRenderEncoder";
        ...
        // Clamp values to the bloom threshold.
        {
            ...
            [renderEncoder setFragmentBytes:&threshold length:sizeof(float) atIndex:6];
